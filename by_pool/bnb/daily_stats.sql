...
WITH
    ...
    CAKE_WBNB_25bps_active AS (
        ...
        FROM
            CAKE_WBNB_25bps_active_daily
    ),
    -- ADDRESS = 0x5db61A5f05580Cf620a9d0f9266E7432811DC309
    CAKE_WBNB_25bps_passive_daily AS (
        SELECT
            DATE_TRUNC ('day', w.time) AS day_start,
            -- Fees
            SUM(w.fee0) as fee0,
    ...
    CAKE_WBNB_25bps_passive AS (
        ...
        FROM
            CAKE_WBNB_25bps_passive_daily
    ),
    -- ADDRESS = 0xc2cdc718908667F1ce99E69d02a7F420bCDb34C2
    USDT_BUSD_1bps_daily AS (
        SELECT
            DATE_TRUNC ('day', w.time) AS day_start,
            -- Fees
            SUM(w.fee0) as fee0,
    ...
    WBNB_BUSD_5bps AS (
        ...
        FROM
            WBNB_BUSD_5bps_daily
    ),
    -- ADDRESS = 0x39721e66193D8a6eF8052246e95c23B6fBdD5fe5
    WETH_WBNB_5bps_active_daily AS (
        SELECT
            DATE_TRUNC ('day', w.time) AS day_start,
            -- Fees
            SUM(w.fee0) as fee0,
    ...
    WETH_WBNB_5bps_active AS (
        ...
        FROM
            WETH_WBNB_5bps_active_daily
    ),
    -- ADDRESS = 0x57eee8ca983FcA38dFE767070F2cE251D4e5E9e1
    WETH_WBNB_5bps_passive_daily AS (
        SELECT
            DATE_TRUNC ('day', w.time) AS day_start,
            -- Fees
            SUM(w.fee0) as fee0,
    ...
    day_series AS (
        ...
        FROM
            unnest (
                sequence (
                    DATE_TRUNC ('day', timestamp '2023-05-12'),
...
FROM
    day_series
    FULL OUTER JOIN CAKE_WBNB_25bps_active ON day_series.day_start = CAKE_WBNB_25bps_active.day_start
